...
#include "i2c.h"
//基于 HAL 库的 I2C 读写
int i2c_write(uint8_t dev_addr, uint8_t reg_addr, uint8_t length, uint8_t *data) {
    if (HAL_I2C_Mem_Write(&hi2c1, dev_addr << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, length, 100) != HAL_OK) {
        return -1;
    }
    return 0;
}
...
int i2c_read(uint8_t dev_addr, uint8_t reg_addr, uint8_t length, uint8_t *data) {
    if (HAL_I2C_Mem_Read(&hi2c1, dev_addr << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, length, 100) != HAL_OK) {
        return -1;
    }
    return 0;
}

unsigned long get_ms(unsigned long *timestamp) {
